...
nr_inertial = floor(nr_bytes/36);

headers = zeros(1,ceil(nr_inertial));
payload_size = zeros(1,ceil(nr_inertial));
inertial_data = zeros(6,ceil(nr_inertial));
meas_counter = zeros(1,ceil(nr_inertial));
dt = zeros(1,ceil(nr_inertial));
checksums = zeros(1,ceil(nr_inertial));
i=1;
while i<=nr_inertial
    headers(i) = fread(file,1,'uint8');
    payload_size(i) = fread(file,1,'uint8');
    inertial_data(:,i) = fread(file,6,'float',0,'b');
    meas_counter(i) = fread(file,1,'uint32',0,'b');
    dt(i) = fread(file,1,'uint32',0,'b');
